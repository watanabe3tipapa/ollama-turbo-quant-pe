...
use crate::types::{
    EnhanceRequest, EnhanceResponse, GenerateRequest, GenerateResponse, ModelsResponse,
};
use crate::turbo_processor::TurboProcessor;
...
pub async fn enhance_response(Json(req): Json<EnhanceRequest>) -> impl IntoResponse {
    let processor = match TurboProcessor::new(256, 42) {
        Ok(p) => p,
        Err(e) => {
            tracing::error!("Failed to create TurboProcessor: {:?}", e);
            let body = json!({ "error": format!("Failed to initialize TurboProcessor: {}", e) });
            return (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(body)).into_response();
        }
    };
    
    match processor.enhance_response(&req.text) {
        Ok(result) => {
            (axum::http::StatusCode::OK, Json(EnhanceResponse {
                original: result.original,
                enhanced: result.enhanced,
                tokens: result.enhanced_length,
                compression_ratio: result.compression_ratio,
            })).into_response()
        }
        Err(e) => {
            tracing::error!("enhance error: {:?}", e);
            let body = json!({ "error": format!("{}", e) });
            (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(body)).into_response()
        }
    }
