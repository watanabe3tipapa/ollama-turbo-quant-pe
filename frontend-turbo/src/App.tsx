...
export default function App() {
  ...
  const enhanceResponse = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setResponse(data.enhanced)
        setEditedResponse(data.enhanced)
        showToast(`Enhanced! Tokens: ${data.tokens}`, 'success')
      } else {
        const error = await res.json()
        showToast(error.error || 'Enhance failed', 'error')
      }
    } catch (err) {
